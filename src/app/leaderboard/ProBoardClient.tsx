...
export function ProBoardClient({ initialPlayers }: ProBoardClientProps) {
    ...
    return (
        <div className="container mx-auto max-w-4xl relative" role="region" aria-label="Apna Esport Pro Board">
                ...
                {tierOrder.map(tier => (
                    <Card key={tier} className="text-center bg-card/30">
                        <CardContent className="p-3">
                            <p className={cn("font-bold text-sm md:text-base", tierConfig[tier].color)}>{tierConfig[tier].name}</p>
            ...
            <div className="pro-board space-y-3 pro-board-bg p-2 md:p-4 rounded-xl border border-border" id="pro-board">
                {loading ? (
                    ...
                    players.map((player, index) => {
                        ...
                        return (
                            <div
                                key={player.uid}
                                className="player-entry flex items-center gap-2 md:gap-4 p-2 md:p-3 bg-card/50 backdrop-blur-sm border border-border rounded-2xl cursor-pointer transition-all duration-300"
                                tabIndex={0}
                            >
                                <div className={cn("rank-badge w-10 h-10 md:w-12 md:h-12 rounded-lg flex items-center justify-center font-bold text-base md:text-lg flex-shrink-0", getRankClass(rank))}>
                                    #${rank}
                                </div>
                                <Avatar className={cn("w-12 h-12 md:w-14 md:h-14 rounded-xl border-2 flex-shrink-0", player.isPremium ? "border-amber-400" : tierConfig[tier].color.replace('text-', 'border-'))}>
                                     <ImageWithFallback 
                                        as={AvatarImage}
                                        user={player}
                                        src={player.photoURL || ''} 
                                ...
                                <div className="player-info flex-grow min-w-0">
                                    <div className="player-name flex items-center gap-2">
                                        <h3 className="text-sm md:text-base font-semibold text-foreground truncate">{player.displayName}</h3>
                                ...
                                <div className="player-score text-right font-bold text-base md:text-lg text-primary min-w-[70px] md:min-w-[80px]">
